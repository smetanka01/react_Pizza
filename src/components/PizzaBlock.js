...
const PizzaBlock = ({name, imageUrl, price, types, sizes}) => {
         ...
         <div className="pizza-block__selector">
            <ul>
               {typesName.map((type, index) => (
                  <li
                     key={type}
                     onClick={(e) => onSelectType(e, index)}
            ...
            <ul>
               {allPizzaSize.map((size, index) => (
                  <li
                     key={size}
                     onClick={(e) => onSelectSize(e, size)}
   ...
   // Строгая типизация данный с помощью плагина prop-types
